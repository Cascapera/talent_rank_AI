...
Progresso: 44 itens em produção · **nada esperando release** · 1 fechado sem correção
           ...
           **a execução não foi confirmada**. Risco real avaliado no checklist do
           ...
           ganhou linha na tabela de execução, nem o 4º release. As surpresas desses
           itens não estão registradas em lugar nenhum.
...
> Não é regressão de refatoração, é feature — mas quem olhar só a série temporal vai
> achar que é.
...
- [~] **R-22** · Proteger o endpoint `/metrics`
  ...
  - [~] Verificado se existe algum scraper consumindo /metrics hoje — **não há
        nada no repositório** (nenhum `prometheus.yml`, nenhum job de scrape no
        `DEPLOY_LIGHTSAIL.md`). Se existir um scraper externo, só o dono do projeto sabe;
        o desenho abaixo torna a resposta indiferente para o deploy.
  - [x] Mudança aplicada (aceita com e sem token) — `METRICS_TOKEN` vazio mantém o
        endpoint aberto; preenchido, exige `X-Metrics-Token` ou `Authorization: Bearer`
  - [ ] Scraper reconfigurado, se houver
  - [ ] Acesso sem token removido — **é config, não código:** preencher `METRICS_TOKEN`
...
- [x] **R-43** 🐛 · Resposta vazia do LLM passava como sucesso, sem retry
  ...
  - [~] Verificado em produção — o caminho feliz não dá para forçar de fora: só aparece
        quando o Gemini devolver vazio de novo. O que se sabe é que **o erro que motivou
        o item não volta em silêncio**: ou o retry salva, ou chega mensagem legível.
  - [x] Commitado — `c14699e`
  - Status: **em produção** · Notas: **o characterization test do R-07 pegou um
...
Uma tarde operando a tela rendeu **quatro defeitos reais**, e nenhum apareceria em teste
automatizado. Vale registrar porque muda a prioridade das próximas rodadas:

| Achado | Como só a tela poderia mostrar |
|---|---|
| **R-43** — resposta vazia do LLM sem retry | precisou de uma importação real falhar 1 em 5 |
| **parecer sobrescrito por vazio** | veio de brinde, pelo characterization test do R-07 |
| **R-44** — o poll morre quando o servidor reinicia | precisou de um `restart` de verdade com a tela aberta |
| **R-42** — a tela nunca diz qual arquivo falhou | é ausência, não erro: nada quebra, só falta |

O R-44 é o mais eloquente: o R-20b tinha **10 testes verdes** e nenhum podia pegá-lo —
eles testam o payload que o backend devolve, e o defeito era do lado que pergunta.
...
| **R-39** | achado, não compromisso | métricas zeram a cada restart e são por worker. **Correção de 2026-08-21:** o item dizia "hoje roda 1 worker, então o segundo problema está latente" — eram **3**, e a inconsistência estava ativa. Depois do incidente de memória a configuração passou a **1 worker com 4 threads**, então voltou a ser latente de fato. Segue para fechar sem correção enquanto ninguém consumir `/metrics/`; no dia em que alguém consumir, o modo multiprocess é obrigatório desde o primeiro dia |
| **R-49** | 🆕 **achado grave em 2026-08-21** | **a instância está subdimensionada:** 416 MiB de RAM com PostgreSQL na mesma máquina. Rodava 3 workers do gunicorn (~150 MB de RSS cada) e morreu por OOM no primeiro login depois de um reboot. Contornado hoje com swap de 2 GB (que **já existia no disco, desativado**) e 1 worker com 4 threads; importação real passou. **Contorno, não conserto** — a recomendação é subir o plano do Lightsail para 1 ou 2 GB, porque o ponto de falha é justamente o fluxo principal da Bruna |
